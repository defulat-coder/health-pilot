...
Check what already exists. PRODUCT.md and DESIGN.md live at the project root, or under `.codex/context/`, `.agents/context/`, or `docs/` (case-insensitive). Read whichever are present with your native file tool. Also note whether `.impeccable/live/config.json` already exists (Step 6 leaves it untouched if so).
