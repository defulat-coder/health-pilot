...
from tools.apple_health_analyzer import latest_report_context
...
def get_user_instructions(run_context: RunContext = None) -> str:
    ...
    db = SessionLocal()
    try:
        report_context = latest_report_context(user_id)
...
如果用户当前的输入看起来是在回复这句话，请你顺着这个语境继续对话。否则正常回答。"""
        ...
        report_block = f"\n\n{report_context}" if report_context else ""
        return COACH_INSTRUCTIONS + user_context + push_context + report_block
    finally:
        db.close()
