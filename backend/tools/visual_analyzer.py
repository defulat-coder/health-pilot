...
from tools.data_analyzer import get_daily_summary
...
class VisualAnalyzer(Toolkit):
    ...
    def analyze_menu_image(self, image_url_or_path: str, context: Optional[str] = None, run_context: Optional[RunContext] = None) -> str:
        ...
        # 1. 获取用户今日摘要（含已摄入、目标、热量缺口等）
        try:
            today_summary = get_daily_summary(run_context, date="") if run_context else "无法获取今日数据。"
            remaining_calories = "未知"
            if "热量缺口" in today_summary:
                parts = today_summary.split("热量缺口：")
                if len(parts) > 1:
